...
```bash
npm run package   # → manim-dock-0.2.1.vsix (gitignored)
```

1. **GitHub Release (fastest public download)**  
   Push `main`, tag `v0.2.1`, attach the `.vsix`. Users: Extensions → Install from VSIX, or Cursor/VS Code can install from the release URL.

2. **Open VSX (Cursor / VSCodium friendly)**  
   Create a publisher at [open-vsx.org](https://open-vsx.org/), then:
   ```bash
   npx ovsx publish manim-dock-0.2.1.vsix -p <OPEN_VSX_TOKEN>
   ```
...
3. **VS Code Marketplace (largest reach)**  
   ...
   npx vsce publish
   ```
   Or publish a built file: `npx vsce publish --packagePath manim-dock-0.2.1.vsix`
