...
```bash
npm run package   # → manim-dock-0.2.5.vsix (gitignored)
```
...
5. Publish:
   ...
   npx ovsx publish manim-dock-0.2.5.vsix -p "$OVSX_PAT"
   ```
...
3. Publish:
   ...
   npx vsce publish --packagePath manim-dock-0.2.5.vsix -p "$VSCE_PAT"
   ```
